...
void SBUSRegister_MSCOM_t::writeWord(const Context& context, u32 value)
{
	if (context != IOP)
...
void SBUSRegister_MSFLG_t::writeWord(const Context& context, u32 value)
{
	if (context == EE)
...
void SBUSRegister_SMFLG_t::writeWord(const Context& context, u32 value)
{
	if (context == EE)
		Register32_t::writeWord(context, readWord(context) & (~value));
	else if (context == IOP)
...
u16 SBUSRegister_F240_t::readHword(const Context& context, size_t arrayIndex)
{
...
u32 SBUSRegister_F240_t::readWord(const Context& context)
{
	if (context == EE)
...
void SBUSRegister_F240_t::writeHword(const Context& context, size_t arrayIndex, u16 value)
{
	if (context == IOP && arrayIndex == 0)
	{
		u16 temp = value & 0xF0;
...
void SBUSRegister_F240_t::writeWord(const Context& context, u32 value)
{
	if (context == EE)
	{
...
u32 SBUSRegister_F300_t::readWord(const Context& context)
{
	throw std::runtime_error("SBUS_F300 not implemented.");
}

void SBUSRegister_F300_t::writeWord(const Context& context, u32 value)
{
	throw std::runtime_error("SBUS_F300 not implemented.");
}
