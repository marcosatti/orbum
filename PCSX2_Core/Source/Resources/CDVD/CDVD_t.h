...
- STATUS controls what the emulator is currently doing: ie: nothing, seeking, reading, etc. 
  This is set upon writing to the N_2004 register, where it also resets the INTR_STAT.CmdComplete bit. Use this in order to step the state within the emulator.
...
public:
	...
	CDVD Registers.
	*/
	std::shared_ptr<Register8_t>  N_2004;     
	std::shared_ptr<Register8_t>  N_2005;
	std::shared_ptr<Register8_t>  N_2006;
	...
	std::shared_ptr<Register8_t>  S_2016;     
	std::shared_ptr<Register8_t>  S_2017;   
	std::shared_ptr<Register8_t>  S_2018;    
