...
IOPCoreCOP0_t::IOPCoreCOP0_t(const PS2Resources_t* const PS2Resources) : 
	...
	Context(std::make_shared<IOPCore_COP0RegisterContext_t>()),
	...
	Status(std::make_shared<IOPCore_COP0RegisterStatus_t>()),
	Cause(std::make_shared<IOPCore_COP0RegisterCause_t>()),
	EPC(std::make_shared<COP0RegisterEPC_t>()),
	PRId(std::make_shared<IOPCore_COP0RegisterPRId_t>()),
...
bool IOPCoreCOP0_t::isCoprocessorUsable() const
{
	...
	if (isOperatingKernelMode())
		return true;
	else if ((Status->getFieldValue(IOPCore_COP0RegisterStatus_t::Fields::CU) & 0x1) > 0)
		return true;
	else
		return false;
}
...
{
	const u32& KUc = Status->getFieldValue(IOPCore_COP0RegisterStatus_t::Fields::KUc);
