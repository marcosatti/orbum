...
void Timers_t::raiseTimerEventPS2CLK(const u32 & cycles)
{
	...
	{
		TimerEventQueue->push(TimerEvent_t(TimerEvent_t::ClockSource_t::BUSCLK));
	}
	else if (mNumPS2CLKEvents % 32 == 0) // Check for BUSCLK / 16 (PS2CLK / 32) event.
	{
		TimerEventQueue->push(TimerEvent_t(TimerEvent_t::ClockSource_t::BUSCLK16));
	}
	else if (mNumPS2CLKEvents % 512 == 0) // Check for BUSCLK / 256 (PS2CLK / 512) event.
	{
		TimerEventQueue->push(TimerEvent_t(TimerEvent_t::ClockSource_t::BUSCLK256));
	}
}

void Timers_t::raiseTimerEventHBLNK()
{
	TimerEventQueue->push(TimerEvent_t(TimerEvent_t::ClockSource_t::HBLNK));
}
