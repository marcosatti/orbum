...
public:
	...
	TODO: Check the thread safety, and may need to look into prioritisation (extend the std::queue class).

	For queuing an exception, use the push() function, for getting the oldest exception (FIFO queue), use front() followed by pop().
	See the EE Core execution core's (ie: InterpreterEECore.cpp) for how they are used.
	*/
