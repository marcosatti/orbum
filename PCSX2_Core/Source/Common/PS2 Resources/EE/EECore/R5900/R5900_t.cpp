...
void R5900_t::setBranchDelayPCIOffset(s16 IInstructionOffset, u8 cycles)
{
	...
	mIsInBranchDelay = true;
	mBranchDelayCycles = cycles;
	mBranchDelayPCTarget = (PC->getPCValue() + Constants::SIZE_MIPS_INSTRUCTION) + (IInstructionOffset << 2); // New PC = (Current PC + 4) + (JInstructionTarget << 2). See for example the instruction BGEZALL for more info.
}
