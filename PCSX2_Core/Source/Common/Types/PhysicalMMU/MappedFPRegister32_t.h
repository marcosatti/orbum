...
class MappedFPRegister32_t : public PhysicalMapped
{
public:
	explicit MappedFPRegister32_t(const u32& physicalAddress, const std::shared_ptr<FPRegister32_t> & fpRegister32);
	virtual ~MappedFPRegister32_t();

	/*
	For 32-bit FPRegisters, only read/writeWord is allowed. All others throw a runtime exception.
