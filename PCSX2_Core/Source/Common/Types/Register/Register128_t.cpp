...
u64 Register128_t::readDword(const System_t context, size_t arrayIndex)
{
	...
	{
#if DEBUG_LOG_VALUE_AS_HEX
			log(Debug, "%s: %s Read u64[%d], Value = 0x%X.", getSystemStr(context), mMnemonic.c_str(), arrayIndex, UD[arrayIndex]);
#else
			log(Debug, "%s: %s Read u64[%d], Value = %d.", getSystemStr(context), mMnemonic.c_str(), arrayIndex, UD[arrayIndex]);
#endif
	}
#endif
...
void Register128_t::writeDword(const System_t context, size_t arrayIndex, u64 value)
{
	...
	{
#if DEBUG_LOG_VALUE_AS_HEX
		log(Debug, "%s: %s Write u64[%d], Value = 0x%X.", getSystemStr(context), mMnemonic.c_str(), arrayIndex, UD[arrayIndex]);
#else
		log(Debug, "%s: %s Write u64[%d], Value = %d.", getSystemStr(context), mMnemonic.c_str(), arrayIndex, UD[arrayIndex]);
#endif
	}
#endif
}
...
u128 Register128_t::readQword(const System_t context)
{
	...
	{
#if DEBUG_LOG_VALUE_AS_HEX
		log(Debug, "%s: %s Read u128, ValueLSB = 0x%X, ValueMSB = 0x%X.", getSystemStr(context), mMnemonic.c_str(), UQ.lo, UQ.hi);
#else
		log(Debug, "%s: %s Read u128, ValueLSB = %d, ValueMSB = %d.", getSystemStr(context), mMnemonic.c_str(), UQ.lo, UQ.hi);
#endif
	}
#endif
...
void Register128_t::writeQword(const System_t context, u128 value)
{
	...
	{
#if DEBUG_LOG_VALUE_AS_HEX
		log(Debug, "%s: %s Write u128, ValueLSB = 0x%X, ValueMSB = 0x%X.", getSystemStr(context), mMnemonic.c_str(), UQ.lo, UQ.hi);
#else
		log(Debug, "%s: %s Write u128, ValueLSB = %d, ValueMSB = %d.", getSystemStr(context), mMnemonic.c_str(), UQ.lo, UQ.hi);
#endif
	}
#endif
}
...
f32 Register128_t::readFloat(const System_t context, size_t arrayIndex)
{
	...
	{
#if DEBUG_LOG_VALUE_AS_HEX
		log(Debug, "%s: %s Read f32[%d], Value = %A.", getSystemStr(context), mMnemonic.c_str(), arrayIndex, F[arrayIndex]);
#else
		log(Debug, "%s: %s Read f32[%d], Value = %F.", getSystemStr(context), mMnemonic.c_str(), arrayIndex, F[arrayIndex]);
#endif
	}
#endif
...
void Register128_t::writeFloat(const System_t context, size_t arrayIndex, f32 value)
{
	...
	{
#if DEBUG_LOG_VALUE_AS_HEX
		log(Debug, "%s: %s Write f32[%d], Value = %A.", getSystemStr(context), mMnemonic.c_str(), arrayIndex, F[arrayIndex]);
#else
		log(Debug, "%s: %s Write f32[%d], Value = %F.", getSystemStr(context), mMnemonic.c_str(), arrayIndex, F[arrayIndex]);
#endif
	}
#endif
}
