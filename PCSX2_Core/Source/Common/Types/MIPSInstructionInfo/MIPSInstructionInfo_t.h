...
struct MIPSInstructionInfo_t
{
	...
	MIPSInstructionInfo_t(
		...
		const u32                     cycles, 
		const MIPSInstructionInfo_t & (* lookupFunction)(const MIPSInstruction_t& instruction)
	);
	...
	const MIPSInstructionInfo_t &   (* mLookupFuncion)(const MIPSInstruction_t & instruction); // When a sub-class is encountered, this variable points to a lookup function containing the logic needed for accessing the sub-class table properly.
};