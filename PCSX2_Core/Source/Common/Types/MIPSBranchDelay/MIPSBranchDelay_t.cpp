...
void MIPSBranchDelay_t::setBranchDelayPCJRegion(const u32& JInstructionTarget, const u8& cycles)
{
	...
	// New PC = (Current PC + 4)[bits 31-28] | (JInstructionTarget << 2). See for example the instruction JAL for more info.
	mBranchDelayPC = ((mPCRegister32->readWord(Context_t::RAW) + Constants::SIZE_MIPS_INSTRUCTION) & 0xF0000000) + (JInstructionTarget << 2);
}
...
void MIPSBranchDelay_t::setBranchDelayPCIOffset(const s16& IInstructionOffset, const u8& cycles)
{
	...
	// New PC = Current PC + 4 + JInstructionTarget. See for example the instruction BGEZALL for more info.
	mBranchDelayPC = (mPCRegister32->readWord(Context_t::RAW) + Constants::SIZE_MIPS_INSTRUCTION) + (IInstructionOffset << 2); 
}
