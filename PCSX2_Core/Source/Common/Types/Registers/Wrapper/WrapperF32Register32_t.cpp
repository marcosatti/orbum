...
u8 WrapperF32Register32_t::readByte(const Context& context, size_t arrayIndex)
{
	throw std::runtime_error("Only Word R/W wrapper calls to FPRegister32_t are allowed.");
}

void WrapperF32Register32_t::writeByte(const Context& context, size_t arrayIndex, u8 value)
{
	throw std::runtime_error("Only Word R/W wrapper calls to FPRegister32_t are allowed.");
}

u16 WrapperF32Register32_t::readHword(const Context& context, size_t arrayIndex)
{
	throw std::runtime_error("Only Word R/W wrapper calls to FPRegister32_t are allowed.");
}

void WrapperF32Register32_t::writeHword(const Context& context, size_t arrayIndex, u16 value)
{
	throw std::runtime_error("Only Word R/W wrapper calls to FPRegister32_t are allowed.");
}

u32 WrapperF32Register32_t::readWord(const Context& context)
{
	return mFPRegister32->readWord(context);
}

void WrapperF32Register32_t::writeWord(const Context& context, u32 value)
{
