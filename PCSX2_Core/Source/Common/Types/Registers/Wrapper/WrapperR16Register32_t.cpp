...
u8 WrapperR16Register32_t::readByte(const Context& context, size_t arrayIndex)
{
	throw std::runtime_error("Only Word R/W wrapper calls to Register16_t are allowed.");
}

void WrapperR16Register32_t::writeByte(const Context& context, size_t arrayIndex, u8 value)
{
	throw std::runtime_error("Only Word R/W wrapper calls to Register16_t are allowed.");
}

u16 WrapperR16Register32_t::readHword(const Context& context, size_t arrayIndex)
{
	throw std::runtime_error("Only Word R/W wrapper calls to Register16_t are allowed.");
}

void WrapperR16Register32_t::writeHword(const Context& context, size_t arrayIndex, u16 value)
{
	throw std::runtime_error("Only Word R/W wrapper calls to Register16_t are allowed.");
}

u32 WrapperR16Register32_t::readWord(const Context& context)
{
	return static_cast<u32>(mRegister16->readHword(context));
}

void WrapperR16Register32_t::writeWord(const Context& context, u32 value)
{
