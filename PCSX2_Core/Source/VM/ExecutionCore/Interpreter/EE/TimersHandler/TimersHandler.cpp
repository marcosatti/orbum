...
s64 TimersHandler::executionStep(const ClockSource_t & clockSource)
{
	...
	for (auto i = 0; i < PS2Constants::EE::Timers::NUMBER_TIMERS; i++)
	{
		...
		if (!Timers->EE_TIMERS[i]->mMODE->getFieldValue(EETimersTimerRegister_Mode_t::Fields::CUE))
			continue;
		...
		if (isTimerCLKSEqual(i, clockSource))
		{
			...
			if (Timers->EE_TIMERS[i]->mMODE->getFieldValue(EETimersTimerRegister_Mode_t::Fields::GATE) && !isTimerGateSpecialHBLNK(i))
			{
				...
				if (Timers->EE_TIMERS[i]->mMODE->getFieldValue(EETimersTimerRegister_Mode_t::Fields::GATM) == 0)
				{
					if (!gateSources[Timers->EE_TIMERS[i]->mMODE->getFieldValue(EETimersTimerRegister_Mode_t::Fields::GATS)])
						Timers->EE_TIMERS[i]->mCOUNT->increment(1);
				}
				else
				{
					Timers->EE_TIMERS[i]->mCOUNT->increment(1);
...
{
	auto& timer = getVM()->getResources()->EE->Timers->EE_TIMERS[timerNumber];
...
{
	auto& timerRegister = getVM()->getResources()->EE->Timers->EE_TIMERS[timerNumber];
...
{
	if (getVM()->getResources()->EE->Timers->EE_TIMERS[timerNumber]->mMODE->getFieldValue(EETimersTimerRegister_Mode_t::Fields::CLKS) == 3 &&
		getVM()->getResources()->EE->Timers->EE_TIMERS[timerNumber]->mMODE->getFieldValue(EETimersTimerRegister_Mode_t::Fields::GATS) == 0)
	{
		return true;
	}
	else
