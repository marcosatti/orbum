...
s64 DMACInterpreter::executionStep(const ClockSource_t & clockSource)
{
	...
	auto& Channels = DMAC->DMAC_CHANNELS;

	// Check if DMA transfers are enabled.
	if (!DMAC->DMAC_REGISTER_D_CTRL->getFieldValue(EEDmacRegister_CTRL_t::Fields::DMAE))
		return 1;
...
{
	auto& DMAC = getVM()->getResources()->EE->DMAC;
	auto& Channel = DMAC->DMAC_CHANNELS[mChannelIndex];
...
void DMACInterpreter::suspendTransfer() const
{
	...
	DMAC->DMAC_REGISTER_D_STAT->setFieldValue(key, 1);
...
bool DMACInterpreter::isSourceStallControlOn() const
{
	...
	{
		const u32 STS = DMAC->DMAC_REGISTER_D_CTRL->getFieldValue(EEDmacRegister_CTRL_t::Fields::STS);
...
bool DMACInterpreter::isDrainStallControlOn() const
{
	...
	{
		const u32 STD = DMAC->DMAC_REGISTER_D_CTRL->getFieldValue(EEDmacRegister_CTRL_t::Fields::STD);
...
void DMACInterpreter::updateSourceStallControlAddress() const
{
	...
	DMAC->DMAC_REGISTER_D_STADR->setFieldValue(EEDmacRegister_STADR_t::Fields::ADDR, MADR);
}
...
bool DMACInterpreter::isDrainStallControlWaiting() const
{
	...
	const u32 MADR = Channel->mMADR->getFieldValue(EEDmacChannelRegister_MADR_t::Fields::ADDR);
	const u32 STADR = DMAC->DMAC_REGISTER_D_STADR->getFieldValue(EEDmacRegister_STADR_t::Fields::ADDR);

	if ((MADR + 8) > STADR)
	{
		DMAC->DMAC_REGISTER_D_STAT->setFieldValue(EEDmacRegister_STAT_t::Fields::SIS, 1);
		return true;
	}
...
u128 DMACInterpreter::readDataChannel() const
{
	// TODO: implement.
	throw std::runtime_error("DMA channel read qword not implemented.");
}

void DMACInterpreter::writeDataChannel(u128 data) const
{
	// TODO: implement.
	throw std::runtime_error("DMA channel write qword not implemented.");
}

u128 DMACInterpreter::readDataMemory(u32 PhysicalAddressOffset, bool SPRAccess) const
{
...
void DMACInterpreter::writeDataMemory(u32 PhysicalAddressOffset, bool SPRAccess, u128 data) const
{
	auto& EEMMU = getVM()->getResources()->EE->PhysicalMMU;
...
void DMACInterpreter::checkInterleaveCount() const
{
	...
	if (!Channel->mInterleavedSkipState)
	{
		...
		if (Channel->mInterleavedCountState >= DMAC->DMAC_REGISTER_D_SQWC->getFieldValue(EEDmacRegister_SWQC_t::Fields::TQWC))
		{
	...
	else
	{
		...
		if (Channel->mInterleavedCountState >= DMAC->DMAC_REGISTER_D_SQWC->getFieldValue(EEDmacRegister_SWQC_t::Fields::SQWC))
		{
			// Change to skip mode and reset the count.
			Channel->mInterleavedSkipState = !Channel->mInterleavedSkipState;
			Channel->mInterleavedCountState = 0;
