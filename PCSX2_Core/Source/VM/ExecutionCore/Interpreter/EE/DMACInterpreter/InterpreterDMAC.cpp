...
void InterpreterDMAC::executionStep_Normal(u32 channelID)
{
	...
	if (EE->DMAC->DataCountState[channelID] == QWC)
	{
		// TODO: implement.
		throw std::runtime_error("DataCountState[channelID] == QWC, but not implemented.");
	}
	...
	if (channelProperties.PhysicalMode == PhysicalMode_t::SLICE)
	{
		if (EE->DMAC->DataCountState[channelID] % 8 == 0)
		{
			...
			const u32 addressOffset = EE->DMAC->DataCountState[channelID] * Constants::NUMBER_BYTES_IN_QWORD;
...
void InterpreterDMAC::transferDataUnit(u32 channelID)
{
	auto& EE = getVM()->getResources()->EE;
	auto& channelRegisters = EE->DMAChannelRegisters[channelID];
...
void InterpreterDMAC::executionStep_Chain(u32 channelID)
{
	...
	default:
	{
		throw std::runtime_error("Could not determine chain mode context, or not supported on this channel.");
	}
	}
}
...
DMADataUnit_t InterpreterDMAC::readDataChannel(ChannelID_t channel)
{
	// TODO: implement.
	throw std::runtime_error("DMA channel read qword not implemented.");
}

void InterpreterDMAC::writeDataChannel(ChannelID_t channel, DMADataUnit_t data)
{
	// TODO: implement.
	throw std::runtime_error("DMA channel write qword not implemented.");
}

DMADataUnit_t InterpreterDMAC::readDataMemory(u32 PhysicalAddressOffset, bool SPRAccess)
{
	// TODO: implement.
	throw std::runtime_error("DMA memory read qword not implemented.");
}

void InterpreterDMAC::writeDataMemory(u32 PhysicalAddressOffset, bool SPRAccess, DMADataUnit_t packet)
{
	// TODO: implement.
