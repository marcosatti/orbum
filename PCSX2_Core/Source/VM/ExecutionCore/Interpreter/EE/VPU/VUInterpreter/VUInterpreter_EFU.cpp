#include "stdafx.h"

#include "Common/Global/Globals.h"

#include "VM/ExecutionCore/Interpreter/EE/VPU/VUInterpreter/VUInterpreter.h"

void VUInterpreter::ESADD()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ESADD: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ESADD: Not implemented.");
#endif
}

void VUInterpreter::ERSADD()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ERSADD: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ERSADD: Not implemented.");
#endif
}

void VUInterpreter::ELENG()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ELENG: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ELENG: Not implemented.");
#endif
}

void VUInterpreter::ERLENG()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ERLENG: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ERLENG: Not implemented.");
#endif
}

void VUInterpreter::EATANxy()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) EATANxy: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("EATANxy: Not implemented.");
#endif
}

void VUInterpreter::EATANxz()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) EATANxz: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("EATANxz: Not implemented.");
#endif
}

void VUInterpreter::ESUM()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ESUM: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ESUM: Not implemented.");
#endif
}

void VUInterpreter::ERCPR()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ERCPR: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ERCPR: Not implemented.");
#endif
}

void VUInterpreter::ESQRT()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ESQRT: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ESQRT: Not implemented.");
#endif
}

void VUInterpreter::ERSQRT()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ERSQRT: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ERSQRT: Not implemented.");
#endif
}

void VUInterpreter::ESIN()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) ESIN: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("ESIN: Not implemented.");
#endif
}

void VUInterpreter::EATAN()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) EATAN: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("EATAN: Not implemented.");
#endif
}

void VUInterpreter::EEXP()
{
	// TODO: Implement.
#if defined(BUILD_DEBUG)
	logDebug("(%s, %d) EEXP: Not implemented.", __FILENAME__, __LINE__);
#else
	throw std::runtime_error("EEXP: Not implemented.");
#endif
}
