...
void IOPCoreInterpreter::handleException(const IOPCoreException_t & exception)
{
	...
	logDebug("IOPCore ExceptionHandler called! Type = %s", exceptionProperties->mMnemonic);
#endif
	...
	{
		u32 pcValue = PC->readWord(IOP) - Constants::SIZE_MIPS_INSTRUCTION * 2;
	...
	{
		u32 pcValue = PC->readWord(IOP) - Constants::SIZE_MIPS_INSTRUCTION;
		COP0->EPC->writeWord(RAW, pcValue);
...
bool IOPCoreInterpreter::getPhysicalAddress(const u32& virtualAddress, const MMUAccess_t& access, u32& physicalAddress)
{
	...
	{
		// Test for kseg0
		if (virtualAddress >= PS2Constants::MIPS::MMU::VADDRESS_KERNEL_LOWER_BOUND_2 && virtualAddress <= PS2Constants::MIPS::MMU::VADDRESS_KERNEL_UPPER_BOUND_2)
		{
			...
			physicalAddress = (virtualAddress - PS2Constants::MIPS::MMU::VADDRESS_KERNEL_LOWER_BOUND_2);
			return false;
		}

		// Test for kseg1
		if (virtualAddress >= PS2Constants::MIPS::MMU::VADDRESS_KERNEL_LOWER_BOUND_3 && virtualAddress <= PS2Constants::MIPS::MMU::VADDRESS_KERNEL_UPPER_BOUND_3)
		{
			// We are in kseg1, so to get the physical address we just minus the kseg1 base address of 0xA0000000.
			physicalAddress = (virtualAddress - PS2Constants::MIPS::MMU::VADDRESS_KERNEL_LOWER_BOUND_3);
			return false;
		}
		...
		if (virtualAddress >= PS2Constants::IOP::IOPCore::MMU::VADDRESS_SPECIAL_2_LOWER_BOUND)
		{
		...
		if (virtualAddress <= PS2Constants::IOP::IOPCore::MMU::VADDRESS_SPECIAL_1_UPPER_BOUND)
		{
