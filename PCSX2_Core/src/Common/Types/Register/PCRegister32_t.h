...
{
public:
	PCRegister32_t(const char * mnemonic, const bool debugReads, const bool debugWrites);

	/*
	Handles the updating of the PC by either jumping to the branch location or incrementing the PC by 4 (next instruction).
	*/
	void next(const Context_t context);

	/*
	Set the PC to a relative value immediately.
	*/
	void setPCValueRelative(const Context_t context, const s32 relativePC);

	/*
	Set the PC to an absolute value immediately.
	*/
	void setPCValueAbsolute(const Context_t context, const u32 absolutePC);
	...
	*/
	...
	void setBranchPCJRegion(const Context_t context, const u32 JInstructionTarget, const int cycles);
	void setBranchPCIOffset(const Context_t context, const s16 IInstructionOffset, const int cycles);

	/*
	Causes the branch delay to execute immediately (useful for pipeline flushes).
	*/
	void doBranchNow(const Context_t context);
