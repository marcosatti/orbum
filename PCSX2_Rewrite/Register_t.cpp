...
u32 Register32_t::getBitRange32(u8 startPos, u8 bitLength)
{
	u32 endPos = startPos + bitLength;
	u32 maskTemp1 = ((u32)~0x0 << startPos);
	...
	// I have not accounted for if startPos >= 32 as it is assumed that the user wouldn't try it :)
	if (endPos >= 32) maskTemp2 = 0;
	else maskTemp2 = (maskTemp2 << endPos);

	u32 mask = (maskTemp1 ^ maskTemp2);
	u32 value = (UW & mask) >> startPos;
	return value;
}

void Register32_t::setBitRange32(u8 startPos, u8 bitLength, u32 value)
{
	u32 endPos = startPos + bitLength;
	u32 maskTemp1 = ((u32)~0x0 << startPos);
	u32 maskTemp2 = ((u32)~0x0);
