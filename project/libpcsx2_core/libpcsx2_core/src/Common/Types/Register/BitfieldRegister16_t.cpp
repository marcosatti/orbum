...
{
	return MathUtil::extractMaskedValue16(BitfieldRegister16_t::readHword(context), mFields[fieldIndex].mStartPosition, mFields[fieldIndex].mLength);
}

void BitfieldRegister16_t::setFieldValue(const Context_t context, const int fieldIndex, const u16 value)
{
	writeHword(context, MathUtil::insertMaskedValue16(BitfieldRegister16_t::readHword(context), value, mFields[fieldIndex].mStartPosition, mFields[fieldIndex].mLength));
}
