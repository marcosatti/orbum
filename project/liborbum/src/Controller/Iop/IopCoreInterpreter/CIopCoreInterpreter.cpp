...
#include "Resources/Iop/Core/IOPCoreException_t.h"
...
int IOPCoreInterpreter_s::step(const Event_t & event)
{
	...
	const uptr address = mIOPCore->R3000->PC->read_uword();
	...
	mIOPCoreInstruction = IopCoreInstruction(mByteMMU->read_uword(physicalAddress));

#if defined(BUILD_DEBUG)
	static udword DEBUG_LOOP_BREAKPOINT = 0x100000002A452C;
	...
	if (DEBUG_LOOP_COUNTER >= DEBUG_LOOP_BREAKPOINT)
	{
		...
		log(Debug, "IOPCore cycle 0x%llX: "
			...
			"Instruction = %s",
			DEBUG_LOOP_COUNTER,
			pcAddress, mIOPCore->R3000->PC->isBranchPending(), !mIOPCore->COP0->Status->is_interrupts_masked(),
...
void IOPCoreInterpreter_s::handleInterruptCheck()
{
	...
	{
		uword ipCause = COP0->Cause->getFieldValue(, IopCoreCop0Register_Cause::IP);
		uword imStatus = COP0->Status->getFieldValue(, IopCoreCop0Register_Status::IM);
		if (ipCause & imStatus)
		{
...
void IOPCoreInterpreter_s::printInterruptInfo() const
{
	...
	log(Debug, "IOPCore interrupt exception occurred @ cycle = 0x%llX, PC = 0x%08X, BD = %d. Printing interrupt sources:",
		DEBUG_LOOP_COUNTER, mIOPCore->R3000->PC->read_uword(), mIOPCore->R3000->PC->isBranchPending());

	// Check the INTC.
	for (auto& irqField : IOPIntcRegister_STAT_t::IRQ_KEYS)
	{
	...
	if (STAT->getFieldValue(, IOPIntcRegister_STAT_t::DMAC) 
		& MASK->getFieldValue(, IOPIntcRegister_MASK_t::DMAC))
	{
		...
		{
			if (ICR0->getFieldValue(, IopDmacRegister_Icr0::CHANNEL_TCI_KEYS[i])
				&& ICR0->getFieldValue(, IopDmacRegister_Icr0::CHANNEL_TCM_KEYS[i]))
			{
		...
		for (int i = 0; i < half; i++)
		{
			if (ICR1->getFieldValue(, IopDmacRegister_Icr1::CHANNEL_TCI_KEYS[i])
				&& ICR1->getFieldValue(, IopDmacRegister_Icr1::CHANNEL_TCM_KEYS[i]))
			{
...
{
	if (!mIOPCore->COP0->is_usable())
	{
		// Coprocessor was not usable. Raise an exception.
		mIOPCore->COP0->Cause->setFieldValue(, IopCoreCop0Register_Cause::CE, 0);
...
void IOPCoreInterpreter_s::handleException(const IOPCoreException_t & exception)
{
	...
	COP0->Cause->setFieldValue(, IopCoreCop0Register_Cause::ExcCode, exceptionProperties->mExeCode);
	...
	if (PC->isBranchPending())
	{
		...
		COP0->Cause->setFieldValue(, IopCoreCop0Register_Cause::BD, 1);
		PC->resetBranch();
	}
	else
	{
		...
		COP0->EPC->write_uword(pcValue);
		COP0->Cause->setFieldValue(, IopCoreCop0Register_Cause::BD, 0);
	}
	...
	if (COP0->Status->getFieldValue(, IopCoreCop0Register_Status::BEV) == 1)
...
bool IOPCoreInterpreter_s::getPhysicalAddress(const uptr address)
{
	auto& COP0 = mIOPCore->COP0;
	...
	if (virtualAddress == DEBUG_VA_BREAKPOINT)
	{
		log(Debug, "IOP MMU breakpoint hit @ cycle = 0x%llX, PC = 0x%08X, VA = 0x%08X (%s).",
			DEBUG_LOOP_COUNTER, mIOPCore->R3000->PC->read_uword(), DEBUG_VA_BREAKPOINT, (access == READ) ? "READ" : "WRITE");
	}
#endif
	...
	if (context == MipsCoprocessor0::OperatingContext::Kernel)
	{
		...
		if (virtualAddress <= Constants::IOP::IOPCore::MMU::VADDRESS_SPECIAL_1_UPPER_BOUND)
		{
			...
			if (COP0->Status->getFieldValue(, IopCoreCop0Register_Status::IsC) && access == WRITE)
			{
