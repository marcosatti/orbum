...
EECoreInterpreter_s::EECoreInterpreter_s(VM * vm, const VUInterpreter_s & vuInterpreter) :
	VMSystem_t(vm, BusContext::Ee),
...
int EECoreInterpreter_s::step(const Event_t & event)
{
	...
	const uptr address = mEECore->R5900->PC->read_uword();
	...
	mEECoreInstruction = EeCoreInstruction(mByteMMU->read_uword(physicalAddress));

#if defined(BUILD_DEBUG)
	static udword DEBUG_LOOP_BREAKPOINT = 0x100000004F94E00;
	static uword DEBUG_PC_BREAKPOINT = 0x0;
	if (DEBUG_LOOP_COUNTER >= DEBUG_LOOP_BREAKPOINT)
	{
		...
		log(Debug, "EECore cycle = 0x%llX: "
			...
			"Instruction = %s",
			DEBUG_LOOP_COUNTER,
			pcAddress, mEECore->R5900->PC->isBranchPending(), !mEECore->COP0->Status->is_interrupts_masked(),
...
void EECoreInterpreter_s::handleInterruptCheck()
{
	...
	{
		uword ipCause = COP0->Cause->getFieldValue(, EeCoreCop0Register_Cause::IP);
		uword imStatus = COP0->Status->getFieldValue(, EeCoreCop0Register_Status::IM);
		if (ipCause & imStatus)
		{
...
void EECoreInterpreter_s::printInterruptInfo() const
{
	...
	log(Debug, "EECore interrupt exception occurred @ cycle = 0x%llX, PC = 0x%08X, BD = %d. Printing interrupt sources:",
		DEBUG_LOOP_COUNTER, mEECore->R5900->PC->read_uword(), mEECore->R5900->PC->isBranchPending());

	// Check the INTC.
	if ((ipCause & imStatus) & 0x4)
	{
		for (auto& irqField : EeIntcRegister_Stat::IRQ_KEYS)
		{
	...
	if ((ipCause & imStatus) & 0x8)
	{
		...
		{
			if (STAT->getFieldValue(, EeDmacRegister_Stat::CHANNEL_CIS_KEYS[i])
				&& STAT->getFieldValue(, EeDmacRegister_Stat::CHANNEL_CIM_KEYS[i]))
			{
...
void EECoreInterpreter_s::handleCountEventCheck() const
{
	...
	if (mEECore->COP0->Count->read_uword() == mEECore->COP0->Compare->read_uword())
		mEECore->COP0->Cause->set_irq_line(, 7);
	else
		mEECore->COP0->Cause->clear_irq_line(, 7);
}

bool EECoreInterpreter_s::handleCOP0Usable()
{
	if (!mEECore->COP0->is_usable())
	{
		// Coprocessor was not usable. Raise an exception.
		mEECore->COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::CE, 0);
...
{
	if (!mEECore->FPU->is_usable())
	{
		// Coprocessor was not usable. Raise an exception.
		mEECore->COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::CE, 1);
		handleException(EECoreException_t::EX_COPROCESSOR_UNUSABLE);
		return true;
	}
...
bool EECoreInterpreter_s::handleCOP2Usable()
{
	...
	{
		// Coprocessor was not usable. Raise an exception.
		mEECore->COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::CE, 2);
		handleException(EECoreException_t::EX_COPROCESSOR_UNUSABLE);
		return true;
	}
...
void EECoreInterpreter_s::handleException(const EECoreException_t& exception)
{
	...
	{
		COP0->Status->setFieldValue(, EeCoreCop0Register_Status::ERL, 1);
		COP0->Status->setFieldValue(, EeCoreCop0Register_Status::BEV, 1);
	}
...
void EECoreInterpreter_s::handleException_L1() const
{
	...
	uword vectorOffset = 0x0;

	// Set Cause.ExeCode value.
	COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::ExcCode, mExceptionProperties->mExeCode);
	...
	if (COP0->Status->getFieldValue(, EeCoreCop0Register_Status::EXL) == 1)
	{
	...
	else
	{
		...
		{
			uword pcValue = PC->read_uword() - Constants::MIPS::SIZE_MIPS_INSTRUCTION;
			COP0->EPC->write_uword(pcValue);
			COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::BD, 1);
			PC->resetBranch(); // Reset branch delay slot.
		}
		else
		{
			uword pcValue = PC->read_uword();
			COP0->EPC->write_uword(pcValue);
			COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::BD, 0);
		}

		// Set to kernel mode and disable interrupts.
		COP0->Status->setFieldValue(, EeCoreCop0Register_Status::EXL, 1);
	...
	if (COP0->Status->getFieldValue(, EeCoreCop0Register_Status::BEV) == 1)
	{
...
void EECoreInterpreter_s::handleException_L2() const
{
	...
	COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::EXC2, mExceptionProperties->mExeCode);
	...
	if (PC->isBranchPending())
	{
		...
		uword pcValue = PC->read_uword() - Constants::MIPS::SIZE_MIPS_INSTRUCTION * 2;
		COP0->ErrorEPC->write_uword(pcValue);
		COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::BD2, 1);
		PC->resetBranch(); // Reset branch delay slot.
	}
	else
	{
		uword pcValue = PC->read_uword() - Constants::MIPS::SIZE_MIPS_INSTRUCTION;
		COP0->ErrorEPC->write_uword(pcValue);
		COP0->Cause->setFieldValue(, EeCoreCop0Register_Cause::BD2, 0);
	}
	...
	else
	{
		...
		// Select vector base to use and set PC to use the specified vector.
		if (COP0->Status->getFieldValue(, EeCoreCop0Register_Status::DEV) == 1)
		{
			PC->setPCValueAbsolute(, Constants::MIPS::Exceptions::Imp46::VADDRESS_EXCEPTION_BASE_A1 + vectorOffset);
		}
		else
...
void EECoreInterpreter_s::handleMMUError(const uptr address, const MMUAccess_t access, const MMUError_t error, const s32 tlbEntryIndex)
{
	auto& COP0 = mEECore->COP0;
	...
	COP0->BadVAddr->write_uword(virtualAddress);
	COP0->Context->setFieldValue(, EeCoreCop0Register_System::BadVPN2, MathUtil::getHI19(virtualAddress));
	COP0->EntryHi->setFieldValue(, EeCoreCop0Register_EntryHi::VPN2, MathUtil::getHI19(virtualAddress));
	COP0->Random->setFieldValue(, EeCoreCop0Register_Random::Random, TLB->generate_new_tlb_index());

	if (tlbEntryIndex > 0) 
		COP0->EntryHi->setFieldValue(, EeCoreCop0Register_EntryHi::ASID, TLB->tlb_entry_at(tlbEntryIndex).mASID); // Set only if tlb entry index valid.
...
bool EECoreInterpreter_s::getPhysicalAddress(const uptr address)
{
	...
	if (virtualAddress == DEBUG_VA_BREAKPOINT)
	{
		log(Debug, "EE MMU breakpoint hit @ cycle = 0x%llX, PC = 0x%08X, VA = 0x%08X (%s).",
			DEBUG_LOOP_COUNTER, mEECore->R5900->PC->read_uword(), DEBUG_VA_BREAKPOINT, (access == READ) ? "READ" : "WRITE");
	}
#endif
	...
	if (context == MipsCoprocessor0::OperatingContext::Kernel)
	{
		...
		if (COP0->Status->getFieldValue(, EeCoreCop0Register_Status::ERL) == 1) 
		{
	...
	int tlb_index = TLB->find_tlb_entry_index(virtualAddress);
	if (tlbIndex == -1)
	{
		handleMMUError(virtualAddress, access, TLB_REFILL, tlbIndex);
		return true;
	}
	auto& TLBEntry = TLB->tlb_entry_at(tlbIndex);
	...
	if (TLBEntry.mG == 0)
	{
		...
		if (COP0->EntryHi->getFieldValue(, EeCoreCop0Register_EntryHi::ASID) != TLBEntry.mASID)
		{
			handleMMUError(virtualAddress, access, TLB_REFILL, tlbIndex);
			return true;
		}
	...
	if (TLBEntry.mS)
	{
		...
		uword offset16KB = virtualAddress & Constants::MASK_16KB;
	...
	uword indexEvenOddMask = ((TLBEntry.mMask << 12) | 0xFFF) + 1;
	ubyte TLBEvenOddIndex = (virtualAddress & indexEvenOddMask) ? 1 : 0;
	...
	*/
	...
	uword offsetMask = (TLBEntry.mMask << 12) | 0xFFF;
