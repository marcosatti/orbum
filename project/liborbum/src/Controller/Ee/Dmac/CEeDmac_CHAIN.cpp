...
void CEeDmac::CHAIN_SRC_CALL(EeDmacChannel & channel) const
{
	...
	if (asp >= 2)
	{
		...
		BOOST_LOG(Core::get_logger()) << 
			boost::format("EE DMAC Channel %d had chain mode stack overflow! Exiting transfer. TADR has been changed.")
			% (*channel.channel_id);
	}
	else
	{
...
void CEeDmac::CHAIN_SRC_RET(EeDmacChannel & channel) const
{
	...
	if (asp == 0)
	{
		channel.chcr->tag_exit = true;
		BOOST_LOG(Core::get_logger()) << 
			boost::format("EE DMAC Channel %d had chain mode stack underflow! Exiting transfer. TADR left unchanged.")
			% (*channel.channel_id);
	}
	else
	{
