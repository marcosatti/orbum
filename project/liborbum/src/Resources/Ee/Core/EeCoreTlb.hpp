...
{
public:
	/// Performs an iterative lookup on the TLB for the given VPN contained in the virtual address.
	...
	int find_tlb_entry_index(const uptr vaddress) const;

	/// Gets the TLB entry at the specified index - use find_tlb_entry_index() to make sure it exists first.
	const EeCoreTlbEntry & tlb_entry_at(int index) const;
