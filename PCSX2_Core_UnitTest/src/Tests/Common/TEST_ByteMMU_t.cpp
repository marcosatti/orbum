...
TEST_F(TEST_ByteMMU_t, READ_WRITE_8BIT) 
{
	...
	ASSERT_EQ(valueU8, mmu.readByte(Context_t::EECore, address));
	mmu.writeByte(Context_t::EECore, address, valueS8);
	ASSERT_EQ(valueS8, static_cast<s8>(mmu.readByte(Context_t::EECore, address)));
}
...
TEST_F(TEST_ByteMMU_t, READ_WRITE_16BIT)
{
	...
	ASSERT_EQ(valueU16, mmu.readHword(Context_t::EECore, address));
	mmu.writeHword(Context_t::EECore, address, valueS16);
	ASSERT_EQ(valueS16, static_cast<s16>(mmu.readHword(Context_t::EECore, address)));
}
...
TEST_F(TEST_ByteMMU_t, READ_WRITE_32BIT)
{
	...
	ASSERT_EQ(valueU32, mmu.readWord(Context_t::EECore, address));
	mmu.writeWord(Context_t::EECore, address, valueS32);
	ASSERT_EQ(valueS32, static_cast<s32>(mmu.readWord(Context_t::EECore, address)));
}
...
TEST_F(TEST_ByteMMU_t, READ_WRITE_64BIT)
{
	...
	ASSERT_EQ(valueU64, mmu.readDword(Context_t::EECore, address));
	mmu.writeDword(Context_t::EECore, address, valueS64);
	ASSERT_EQ(valueS64, static_cast<s64>(mmu.readDword(Context_t::EECore, address)));
}
